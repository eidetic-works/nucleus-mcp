...
20. **Path leakage in heartbeat_status + list_mounted (fixed in `86842144`):** `heartbeat_status` returned absolute plist/timer paths (e.g. `/Users/operator/Library/LaunchAgents/...`). `list_mounted` returned mount configs with absolute paths in command fields. Fix: sanitize all paths by replacing home directory prefix with `~`.
