...
Brain path is injected into the request context via `os.environ["NUCLEAR_BRAIN_PATH"]` before any tool handler executes. This is safe for single-threaded async execution (see Concurrency Contract).
...
| `NUCLEAR_BRAIN_PATH` | *(derived)* | Absolute path to the active brain for the current request. Set automatically; do not set manually unless bypassing tenant resolution entirely. |
